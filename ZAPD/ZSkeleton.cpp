...
std::string ZSkeleton::GetSourceOutputCode(std::string prefix)
{
	if (parent != nullptr)
	{
		for (int i = 0; i < limbs.size(); i++)
		{
			...
			string defaultDLName = StringHelper::Sprintf("%sLimbDL_%06X", name.c_str(), limb->dListPtr);
			...
			if (limb->dListPtr != 0 && parent->GetDeclaration(limb->dListPtr) == nullptr)
			{
				...
				dList->parent = parent;
				dList->SetName(StringHelper::Sprintf("%sLimbDL_%06X", name.c_str(), limb->dListPtr));
			...
			if (typeid(*limb) == typeid(ZLimbLOD))
			{
				...
				if (limbLOD->farDListPtr != 0 && parent->GetDeclaration(limbLOD->farDListPtr) == nullptr)
				{
					...
					dList->parent = parent;
					dList->SetName(StringHelper::Sprintf("%s_farLimbDlist_%06X", name.c_str(), limbLOD->farDListPtr));
					dList->GetSourceOutputCode(defaultPrefix);
				}
