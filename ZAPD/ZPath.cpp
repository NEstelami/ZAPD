                              ...
                              const uint32_t nRawDataIndex)
{
    ...
    if (name == "")
    {
        name = StringHelper::Sprintf("%sPath_%06X", parent->GetName(), rawDataIndex);
    }

    parent->AddDeclarationArray(rawDataIndex, DeclarationAlignment::Align4,
                                pathways.size() * 8, GetSourceTypeName(),
                                name, pathways.size(), "");
}
...
std::string ZPath::GetSourceOutputCode(const std::string& prefix)
{
    ...
    if (decl == nullptr || decl->isPlaceholder)
    {
        parent->AddDeclarationArray(
            rawDataIndex, DeclarationAlignment::Align4,
            pathways.size() * 8, GetSourceTypeName(),
            name, pathways.size(), declaration);
    }
    else
...
PathwayEntry::PathwayEntry(ZFile* nParent)
    : ZResource(nParent)
{
}
...
void PathwayEntry::DeclareReferences(const std::string& prefix)
{
    ...
    for (const auto& point : points)
    {
        ...
        //declaration += StringHelper::Sprintf("\t%s, /* 0x%06X */", point->GetBodySourceCode().c_str(), currentPtr);
        //currentPtr += point->GetRawDataSize();
    ...
    {
        parent->AddDeclarationArray(
            GETSEGOFFSET(listSegmentAddress), DeclarationAlignment::Align4, DeclarationPadding::Pad4,
            points.size() * 6, points.at(0).GetSourceTypeName(),
            StringHelper::Sprintf("%sPathwayList0x%06X", prefix.c_str(), GETSEGOFFSET(listSegmentAddress)),
...
std::string PathwayEntry::GetBodySourceCode() const
{
    ...
    if (Globals::Instance->game == ZGame::MM_RETAIL)
        declaration += StringHelper::Sprintf("	{ %i, %i, %i, %s },",
                                                numPoints, unk1, unk2,
                                                listName.c_str());
    else
        declaration +=
            StringHelper::Sprintf("	{ %i, %s },", numPoints, listName.c_str());
