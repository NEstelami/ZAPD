...
void ZCollisionHeader::ParseRawData()
{
	...
	if (polygons.size() > 0)
	{
		...
		if (polyAddress != 0)
		{
			parent->AddDeclarationArray(
				...
				"CollisionPoly",
				StringHelper::Sprintf("%s_polygons_%08X", name.c_str(), polySegmentOffset), polygons.size(),
				declaration);
		}
	}
	...
	if (polyTypeDefAddress != 0)
		parent->AddDeclarationArray(
			...
			"SurfaceType",
			StringHelper::Sprintf("%s_surfaceType_%08X", name.c_str(), polyTypeDefSegmentOffset), polygonTypes.size(),
			declaration);
	...
	Declaration* decl = parent->AddDeclaration(rawDataIndex, DeclarationAlignment::None, DeclarationPadding::Pad16, 44,
