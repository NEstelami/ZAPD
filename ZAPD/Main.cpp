...
void BuildAssetTexture(const fs::path& pngFilePath, TextureType texType, const fs::path& outPath)
{
	std::string name = outPath.stem();

	ZTexture* tex = ZTexture::FromPNG(pngFilePath, texType);
	std::string cfgPath = StringHelper::Split(pngFilePath, ".")[0] + ".cfg";

	if (File::Exists(cfgPath))
		name = File::ReadAllText(cfgPath);

	std::string src = tex->GetSourceOutputCode(name);
...
void BuildAssetBlob(const fs::path& blobFilePath, const fs::path& outPath)
{
	...
	std::string name = outPath.stem();  // filename without extension
...
void BuildAssetAnimationIntermediette(const fs::path& animPath, const fs::path& outPath)
{
	std::vector<std::string> split = StringHelper::Split(outPath, "/");
